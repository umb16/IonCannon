	...
	class Icons : ScriptableObject
	{
		...
		void CheckFolder()
		{
			foreach (var a in new[] { icons_old, externalmods_old, icons_new, icons_help })
				foreach (var item in a)
					if (item && !AssetDatabase.GetAssetPath(item).StartsWith("Assets/EMX/" + Root.PN_FOLDER + "/Editor/Icons"))
					{
		...
		Texture2D _oldExternalMod(ref string s)
		{
			...
			switch (s)
			{
				...
				case "HIGHLIGHTER_PRESETS_SELECTION": return externalmods_old[35];
				case "HIGHLIGHTER_PRESETS_HL": return externalmods_old[35];
				case "HL_GRADIENT": return externalmods_old[35];
				case "HL_GRADIENT PERSONAL": return externalmods_old[35];
		...
		internal IconData GetHelpTexture(string s)
		{
			...
			var res = new IconData();
			var res_t = icons_help[0];

			if (!res_t)
			{
				Debug.LogWarning("Hierarchy cannot find '" + s + "' HelpTextur texture. Please check 'Editor/Icons/IconsArray.asset'");
				res.SetTexture(Texture2D.whiteTexture);
				return res;
			}
