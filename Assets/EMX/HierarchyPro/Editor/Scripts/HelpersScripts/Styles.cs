...
namespace EMX.HierarchyPlugin.Editor
{
	...
	internal partial class PluginInstance
	{
		...
		{
			if (ReferenceEquals(DEFAUL_SKIN, null)) DEFAUL_SKIN = GUI.skin;
			return DEFAUL_SKIN;
		}
		...
		internal int FONT_8()
		{
			return Mathf.RoundToInt(par_e.PLUGIN_LABELS_FONT_SIZE) - 2;
		}

		internal int FONT_10()
		{
			return Mathf.RoundToInt(par_e.PLUGIN_LABELS_FONT_SIZE);
		}
			...
			get
			{
				if (__STYLE_DEFBUTTON == null)
				{
					...
					/*	if (Root.p[pluginID].ha.hasShowingPrefabHeader)
                        {
                            __STYLE_DEFBUTTON = new GUIStyle( Root.p[pluginID].hierarchyModification.hoveredItemBackgroundStyle ?? throw new Exception("ASD"));
