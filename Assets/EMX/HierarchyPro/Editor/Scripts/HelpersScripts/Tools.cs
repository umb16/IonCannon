...
namespace EMX.HierarchyPlugin.Editor
{
	...
	partial class Tools
	{
		...
		internal static IEnumerable<HierarchyObject> AllSceneObjectsInterator(PluginInstance adapter, bool GetInScenes = false, UnityEngine.Object activeParent = null,
				string extension = null)        // in this example we return less than N if necessary
		{
			...
			if (adapter.pluginID == 0 || GetInScenes)
			{
			...
			if (adapter.pluginID == 1)     //  var LLL = Application.dataPath.Length - "Assets".Length;
			{   // var paths =  System.IO.Directory.GetFiles( Application.dataPath,"*.*",System.IO.SearchOption.AllDirectories ).Where(p=>!p.EndsWith(".meta")).Select(p=>p.Replace('\\','/').Substring(LLL)).ToArray();
				...
				for (int i = 0; i < paths.Length; i++)
				{
					...
					yield return Cache.GetHierarchyObjectByGUID(ref g, ref es);
				}
			}
		}
		static string es = "";
		...
		internal static int AllSceneObjectsInteratorCount(PluginInstance adapter, bool GetInScenes = false)
		{
			if (adapter.pluginID == 1 && !GetInScenes) return -1;
			int res = 0;
			for (int i = 0; i < SceneManager.sceneCount; i++)
			{
	...
	{   // if ( !searchType.IsAssignableFrom( f.FieldType ) ) return false;
		...
		// return f.GetCustomAttributes( serType2 , false ).Length == 0;
			...
			//#EMX_TODO compType.IsAssignableFrom( type ) added for perfomance, but it may affect to any components if playmodekeeper
