	...
	partial class BookmarksforProjectviewModInstance : ExternalModStyles
	{
		...
		void ITEM_ON_DRAG(MouseEventStruct currentStruct)
		{
			...
			{ /*  List<Int32List> targetList = null;
				...
				else // DragAndDrop.PrepareStartDrag();// reset data
				{
					...
					DragAndDrop.objectReferences = currentStruct.hierarchy_obect;
					DragAndDrop.StartDrag("Dragging Assets");
				}
		...
		void MENUCALL(MouseEventStruct currentStruct)
		{
			...
			if (!Application.isPlaying)
			{
				...
				{
					adapter.PUSH_GUI_ONESHOT(() => // var pos = InputData.WidnwoRect(controller.IS_MAIN, Event.current.mousePosition, 190, 68, controller.adapter);
					{
						var pos = new MousePos(Event.current.mousePosition, MousePos.Type.ColorChanger_230_0, true, controller.adapter);
						CategoriesColorsWindowOld.Init(pos, controller.adapter, this, -1);
					});
			...
			menu.AddItem(new GUIContent("Open Bookmarks Settings"), false, () =>
			{
		...
		void DESCRIPTION_ON_UP(MouseEventStruct currentStruct)
		{
			...
			Windows.InputWindow.Init(pos, "Create Description", adapter, action, null, currentStruct.memoryRoot.description);
		...
		void FAVORIT_FOLDERS_ICON_METHOD(MouseEventStruct currentStruct)
		{
			...
			HierarchyCommonData.Instance().SetUndo("Change favorites params");
		...
		void FILTER_ON_DOWN(MouseEventStruct obj)
		{
			...
			for (int i = 0; i < list.Length; i++)
			{
				...
				{
					HierarchyCommonData.Instance().SetUndo("Change filter");
		...
		void REMOVE_ON_UP(MouseEventStruct currentStruct)
		{
			...
			HierarchyCommonData.Instance().SetUndo("Remove favorites params");
			var refBookmarks = GET_BOOKMARKS();
		...
		void CREATE_NEW_CAT(MouseEventStruct currentStruct)
		{
			...
			AddFavCategory(refBookmarks2, VAR_CAT_INDEX, currentStruct.scene, currentStruct.controller);
		}
		...
		void SHOW_STRING(string winname, string startS, Action<string> action, BookProject.FavorControllerWindow controller)
		{
			...
			var adapter = Root.p[0];
			var pos = new MousePos(Event.current.mousePosition, MousePos.Type.Input_190_68, true, adapter);
			Windows.InputWindow.Init(pos, winname, adapter, action, null, startS);
			controller.RepaintNow();
			/*	if (Event.current == null)
				{
					Root.p[0].PUSH_GUI_ONESHOT(result);
		...
		internal void AddFavCategory(List<FolderBookmark> capture_list, int VAR_CAT_INDEX, int scene, BookProject.FavorControllerWindow controller)
		{
			SHOW_STRING("New Category Name", capture_list[VAR_CAT_INDEX].name, (value) =>
			{
				...
				HierarchyCommonData.Instance().SetUndo("New Category");
...
*/
		...
		internal void SHOW_CATEGORY_MENU(ExternalDrawContainer controller, int scene, Func<int, int> CAT_INDEX, GenericMenu _menu = null) //  Debug.Log(curentIndex);
		{
			...
			if (VAR_CAT_INDEX == 0)
			{
				menu.AddDisabledItem(new GUIContent("Rename '" + capture_list[VAR_CAT_INDEX].name + "'"));
				menu.AddDisabledItem(new GUIContent("Remove '" + capture_list[VAR_CAT_INDEX].name + "'Category"));
			}
			else
			{
				...
				menu.AddItem(new GUIContent("Rename '" + capture_list[VAR_CAT_INDEX].name + "'"), false, () =>
				{
					...
					SHOW_STRING("Rename", capture_list[VAR_CAT_INDEX].name, (value) =>
					{
						...
						HierarchyCommonData.Instance().SetUndo("Rename");
				...
				menu.AddItem(new GUIContent("Remove '" + capture_list[VAR_CAT_INDEX].name + "' Category"), false, () =>
				{
						...
						EditorUtility.DisplayDialog("Remove Category?", "Are you sure?", "Yes", "Cancel"))
					{
						...
						HierarchyCommonData.Instance().SetUndo("Remove Category");
			...
			if (_menu == null)
			{
				...
				if (!Application.isPlaying)
				{
					menu.AddSeparator("");
					//	var controller = CURRENT_CONTROLLER;
					menu.AddItem(new GUIContent("Use Background Colors"), adapter.par_e.BOOKMARKS_FOLDER_DRAW_BG_COLOR, () =>
					{
					...
					{
						menu.AddDisabledItem(new GUIContent("Background Colors"));
					}
					else
					{
						menu.AddItem(new GUIContent("Background Colors"), false, () =>
