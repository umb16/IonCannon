...
#pragma warning restore
		...
		void Reset()
		{
			...
			adapter.PUSH_UPDATE_ONESHOT(() => { comps = null; });
		}


		internal int FONT_SIZE()
		{
			return Mathf.RoundToInt(adapter.par_e.HYPERGRAPH_FONTSIZE) - 2;
		}
				...
				for (int i = 0; i < comps.Length; i++)
				{
					...
					for (int fIndex = 0; fIndex < accessor.faList.Length; fIndex++)
					{
						...
						if (Event.current.type == EventType.Repaint)
						{
							...
							if (isnull) GUI.color *= adapter.par_e.HYPERGRAPH_RED_HIGKLIGHTING ? Color.red : alpha;
						...
						if (Event.current.type == EventType.Repaint)
						{
							if (adapter.par_e.HYPERGRAPH_RED_HIGKLIGHTING || EditorGUIUtility.isProSkin) Draw(lineRect, content, HIPERUI_LINE_BLUEGB, false, false, false, false);
		...
		private void DO_BUTTON_FORSCROLL(ExternalDrawContainer controller, Rect rect, GUIContent content, GUIStyle style, int EVENT_ID,
			UnityEngine.Object newGo, Action action = null)
		{
						...
						if (!stateForDrag_B0.HasValue)
						{
							...
							else*/
							{
								var w = CURRENT_WIN;// as _6__BottomWindow_HyperGraphWindow;
													 //controller.tempRoot.
								if (w) controller.tempRoot.PUSH_ONMOUSEUP(rawOnUp);
							}
						}
	...
	/*	private void DEBUG_HORISONTAL(float y, Color color)
		...
		private void StartBroadcasting()
		{
			...
			WASSCAN = true;
			SKANNING = true;
			currentList = Tools.AllSceneObjectsInterator(adapter, true).GetEnumerator();
			CountProgress = Tools.AllSceneObjectsInteratorCount(adapter, true);
			findedList.Clear();
			INPUT_COMPS.Clear();
			currentIndex = 0;
