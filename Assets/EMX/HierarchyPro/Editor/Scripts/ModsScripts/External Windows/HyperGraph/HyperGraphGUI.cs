...
namespace EMX.HierarchyPlugin.Editor.Mods.HyperGraph
{
	...
	partial class HyperGraphModInstance : ExternalModStyles, ICHANGE_SELECTION_OVVERIDE
	{
		...
		public void Update()
		{
			...
			if (CURRENT_WIN.Alive()) CalcBroadCast();
		}
		...
		void RepaintNow()
		{
			if (CURRENT_WIN) CURRENT_WIN.RepaintNow();

			//Debug.Log(CURRENT_WIN.rootVisualElement.Children);
		}
		...
		ExternalDrawContainer CURRENT_CONTROLLER;
		...
		internal void EXTERNAL_HYPER_DRAWER(Rect lineRect, ExternalDrawContainer controller, ExternalModRoot win)
		{
			...
			if (!win) return;
			if (win.position.width != lastWidth || win.position.height != lastHeight)
			{
				lastWidth = win.position.width;
				lastHeight = win.position.height;
				RefreshWithCurrentSelection();
			}
			INIT_STYLES();

			CURRENT_SCALE = adapter.par_e.HYPERGRAPH_SCALE;
		...
		/*
	void DRAG(ExternalDrawContainer controller)
	{
	...
	*/
		...
		void DRAW_INTERFACE_BUTTON(int EVENT_ID, string label, bool ACTIVE, Rect r, ExternalDrawContainer controller)
		{
			H_AR = actions.HOVER(EVENT_ID, r, controller);
			toolbalStyle.Draw(r, label, H_AR, H_AR, H_AR, false);
			if (ACTIVE) adapter.gl.DRAW_TAP_GLOW(r);
		...
		void INTERFACE(ExternalDrawContainer controller)
		{
			...
			TOOLTIP(HR4, "Only refs to UnityEvents will be shown");
			...
			if (Event.current.type == EventType.Repaint)
			{
				...
				for (int i = 0; i < len; i++)
				{
					...
					if (adapter.par_e.HYPERGRAPH_SCANPERFOMANCE == perfomanceArray[i] || H_AR) adapter.gl.DRAW_TAP_GLOW(procrect);
				...
				if (adapter.par_e.HYPERGRAPH_AUTOCHANGE || H_AR) adapter.gl.DRAW_TAP_GLOW(autorefreshrect);
				...
				if (!controller.hide_hierarchy_ui_buttons)
				{
					...
					if (adapter.par_e.HYPERGRAPH_AUTOHIDE || H_AR) adapter.gl.DRAW_TAP_GLOW(hiderect);
				}
				...
				GUI.enabled = en;
				DRAW_INTERFACE_BUTTON(++EVENT_ID, "EVENTS MODE", adapter.par_e.HYPERGRAPH_EVENTS_MODE_BOOL, HR4, controller);
			...
			else
			{
					...
					Event.current.type == EventType.MouseDown)
				{
					...
					actions.ADD_ACTION(EVENT_ID, autorefreshrect, contains => { return false; }, contains =>
					{
						if (contains)
						{
							adapter.par_e.HYPERGRAPH_AUTOCHANGE = !adapter.par_e.HYPERGRAPH_AUTOCHANGE;
						}
					}, controller);
				}
				...
				if (!controller.hide_hierarchy_ui_buttons)
				{
						...
						Event.current.type == EventType.MouseDown)
					{
						...
						actions.ADD_ACTION(EVENT_ID, hiderect, contains => { return false; }, contains =>
						{
							if (contains)
							{
								adapter.par_e.HYPERGRAPH_AUTOHIDE = !adapter.par_e.HYPERGRAPH_AUTOHIDE;
							}
						}, controller);
					}
				}
			...
			if (!adapter.par_e.HYPERGRAPH_AUTOHIDE)
			{
		...
		void MAIN(ExternalDrawContainer controller)
		{
			...
			if (CurrentSelection) DRAW_BOLD_LABEL(titleRect, "HyperGraph: '" + CurrentSelection.ToString() + "'", FONT_SIZE());
			else DRAW_BOLD_LABEL(titleRect, "HyperGraph", FONT_SIZE());
		...
		void SET_SCALE(float _newScale) //  var oldH = par.HiperGraphParams.SCALE;
		{
			...
			olds = adapter.par_e.HYPERGRAPH_SCALE;
			adapter.par_e.HYPERGRAPH_SCALE = _newScale;
			dif = adapter.par_e.HYPERGRAPH_SCALE / olds;

			CURRENT_CONTROLLER.scrollPos.x =
				(CURRENT_CONTROLLER.scrollPos.x -
				 CURRENT_CONTROLLER.WIDTH / 2) * dif +
				CURRENT_CONTROLLER.WIDTH / 2;
			CURRENT_CONTROLLER.scrollPos.y =
					(CURRENT_CONTROLLER.scrollPos.y -
					 CURRENT_CONTROLLER.HEIGHT / 2) * dif +
					CURRENT_CONTROLLER.HEIGHT / 2;
