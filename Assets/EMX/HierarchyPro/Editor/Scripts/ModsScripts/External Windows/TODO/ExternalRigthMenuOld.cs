...
namespace EMX.HierarchyPlugin.Editor.Mods
{
	internal partial class DrawButtonsOld
	{
		...
		internal static void SET_BOOK_WIHTOUT_OBJECTS(ExternalDrawContainer controller, Scene scene, BookObject.BookmarksforGameObjectsModInstance instance)
		{
			...
			SHOW_CATEGORY_MENU(menu, controller, instance, scene, false);
			menu.AddSeparator("");
			menu.AddItem(new GUIContent("Open " + BookmarksforGameObjectsModWindow.NAME + " Settings"), false, () =>
			{
		...
		*/    //menu.AddSeparator("");
		...
		internal static void SET_HIER(GenericMenu menu, ExternalDrawContainer controller, Scene scene)
		{
			...
			menu.AddItem(new GUIContent("+ Create New State"), false, () => { Add_HierExpands(controller, scene); });
		...
		internal static void Add_Scenes(ExternalDrawContainer controller, Scene scene)
		{
			...
			ScenesHistoryModWindow.AddSceneButton_OnSceneChanging(true);
		...
		static void ADD_TO_MENU_LIST_OF_OBJECTS(MemType type, ref GenericMenu menu, ExternalDrawContainer controller, Scene scene)
		{
			bool was = false;
			...
			for (int __c = 0; __c < (type == MemType.Custom ? __list.Count : 1); __c++)
			{
				...
				for (; (type == MemType.Last ? __i >= 0 : __i < memoryRoot.Count) && interator < itemscount;)
				{
					...
					var h = type == MemType.Last || type == MemType.Custom ? memoryRoot[i].gos_get()[0] : null;
					...
					result.Add(new EstimItems()
					{
						...
						active = type == MemType.Scenes ? memoryRoot[i].GuidEquals() : memoryRoot[i].IsSelectedHadrScan(),
		...
		internal static void AddFavCategory(List<BookMarkCategory_Temp> capture_list, int VAR_CAT_INDEX, Scene scene, ExternalDrawContainer controller)
		{
			SHOW_STRING("New Category Name", capture_list[VAR_CAT_INDEX].category_name, (value) =>
			{
				...
				{
					category_name = value,
					bgColor = capture_list[VAR_CAT_INDEX].bgColor
		...
		internal static void AddAndRefreshCustom(UnityEngine.GameObject[] gos, ExternalDrawContainer controller, Scene scene)
		{
			...
			var newObject = new GameObjectMemory(gos, -1, controller.GetCategoryIndex(scene), temp_sc);
		...
		internal static void SHOW_CATEGORY_MENU(GenericMenu menu, ExternalDrawContainer controller, BookObject.BookmarksforGameObjectsModInstance instance, Scene scene, bool disableSwither) //  Debug.Log(curentIndex);
		{
			...
			// if ( adapter.par.SHOW_BOOKMARKS_ROWS && !adapter.par.BOTTOM_AUTOHIDE )
			{
				if (!disableSwither)
				{
					...
					if (!Application.isPlaying)
					{
						...
						{
							adapter.PUSH_UPDATE_ONESHOT(() => AddFavCategory(capture_list, VAR_CAT_INDEX, scene, controller));
						});
					}
			...
			if (!Application.isPlaying)
			{
				...
				//menu.AddSeparator("");
				menu.AddItem(new GUIContent("Use Background Colors"), adapter.par_e.BOOKMARKS_OB_DRAW_BG_COLOR, () =>
				{
				...
				else
				{
					menu.AddItem(new GUIContent("Background Colors"), false, () =>
					{
						adapter.PUSH_GUI_ONESHOT(() => // var pos = InputData.WidnwoRect(controller.IS_MAIN, Event.current.mousePosition, 190, 68, controller.adapter);
						{
							...
							CategoriesColorsWindowOld.Init(pos, controller.adapter, instance, scene);
						});
