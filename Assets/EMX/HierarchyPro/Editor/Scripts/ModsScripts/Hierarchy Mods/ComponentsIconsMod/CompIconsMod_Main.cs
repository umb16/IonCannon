...
namespace EMX.HierarchyPlugin.Editor.Mods
{
    ...
    internal partial class ComponentsIcons_Mod : DrawStackAdapter, ISearchable
    {
        ...
        void PreCalcRect()
        {
            ...
            if (adapter.par_e.USE_OVERRIDE_DEFAULT_ICONS_SIZE) offset.x = adapter.par_e.OVERRIDE_DEFAULT_ICONS_SIZE;
            else offset.x = Window.DefaultIconSize(adapter);
            offset.x += adapter.par_e.COMPONENTS_NEXT_TO_NAME_PADDING + adapter.hierarchyModification.LEFT_PADDING - adapter.hierarchyModification.LEFT_PADDING;
                ...
                if (mon && (DMI || DCI))
                {
                    ...
                    if (DCI)
                    {
                        if (DCI_FS && HierarchyCommonData.Instance().HasCustomIcon(_comps[i]))
                        {
                            ...
                            var load = HierarchyCommonData.Instance()._HasCustomIcon[t.FullName];
