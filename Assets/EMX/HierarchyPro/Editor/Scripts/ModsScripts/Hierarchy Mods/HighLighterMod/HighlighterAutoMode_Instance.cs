...
namespace EMX.HierarchyPlugin.Editor.Mods
{
	internal partial class HighlighterAutoMode_Instance
	{
		...
		internal HighlighterAutoMode_Instance(PluginInstance a, HighlighterMod root)
		{
		...
		internal void SubscribeAutoColorHighLighter(EditorSubscriber sbs)
		{
			...
			if (!adapter.par_e.USE_AUTO_HIGHLIGHTER_MOD) return;
		...
		void m_GetFilter( HierarchyObject o, ref TempColorClass __GetFilterResult)
		{
			...
			var filters = HighLighterCommonData.GetColorFilters;
			...
			for (int f = 0; f < filters.Count; f++)
			{
				...
				{   // if (!__FilterToColor_cache.TryGetValue(filters[f], out getFilterColor)) __FilterToColor_cache.Add(filters[f], getFilterColor = filters[f].AS_TEMPCOLOR_ALIGN_ONLY );
					...
					{   // var asd = ColorFilter.__TempColorClass ;
						...
						ColorFilter.__TempColorClass = new TempColorClass().AssignFromList(ref ColorFilter.__SingleList.list);
