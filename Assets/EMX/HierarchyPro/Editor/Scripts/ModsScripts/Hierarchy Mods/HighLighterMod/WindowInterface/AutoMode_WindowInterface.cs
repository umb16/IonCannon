...
namespace EMX.HierarchyPlugin.Editor.Windows
{
	...
	public partial class HighlighterWindow : Windows.IWindow
	{
		...
		internal class FilterHelper
		{
			...
			internal FilterHelper(EditorWindow w, PluginInstance a)
			{
				ew = w;
				adapter = a;
			}
			...
			void CHANGE_GUI() { HighlighterWindow.CHANGE_GUI(adapter); }
			...
			{   // if ( Event.current.type == EventType.Layout && currentY.Length != Hierarchy_GUI.Instance( a ).ColorFilters.Count ) return 0;
				...
				var customFilters = HighLighterCommonData.GetColorFilters;
			...
			void ToObjectButton(HierarchyObject reference, int index)
			{
				...
				if (index > HighLighterCommonData.GetColorFilters.Count) return;

				var targetFilter = HighLighterCommonData.GetColorFilters[index];
			...
			void FromObjectButton(HierarchyObject reference, int index)
			{
				...
				if (reference != null)
				{
					...
					var cont = HighlighterCache_Icons.GetImageForObject_OnlyCacher(reference);
				...
				if (index >= HighLighterCommonData.GetColorFilters.Count)
				{
					index = HighLighterCommonData.GetColorFilters.Count;
					HighLighterCommonData.GetColorFilters.Add(value);
				}
				else
				{
					oldFilter = HighLighterCommonData.GetColorFilters[index];
					HighLighterCommonData.GetColorFilters[index] = value;
				}

				if (oldFilter != null)
				{
					HighLighterCommonData.GetColorFilters[index].ENABLE = oldFilter.ENABLE;
					HighLighterCommonData.GetColorFilters[index].NAME = oldFilter.NAME;
					HighLighterCommonData.GetColorFilters[index].NameFilter = oldFilter.NameFilter;
					HighLighterCommonData.GetColorFilters[index].ComponentFilter = oldFilter.ComponentFilter;
					HighLighterCommonData.GetColorFilters[index].TagFilter = oldFilter.TagFilter;
					HighLighterCommonData.GetColorFilters[index].LayerFilter = oldFilter.LayerFilter;
				}
			...
			{
				if (index < 0 || index >= HighLighterCommonData.GetColorFilters.Count) return;

				HighLighterCommonData.Undo( "Change Color Filters");

				HighLighterCommonData.GetColorFilters.RemoveAt(index);
			...
			void Swap(int i1, int i2)
			{
				...
				var v1 = HighLighterCommonData.GetColorFilters[i1];
				HighLighterCommonData.GetColorFilters.RemoveAt(i1);

				if (i2 >= HighLighterCommonData.GetColorFilters.Count) HighLighterCommonData.GetColorFilters.Add(v1);
				else HighLighterCommonData.GetColorFilters.Insert(i2, v1);
			...
			private void DrawColoredLabel(Rect labelRect, ColorFilter filter)
			{
				...
				if (filter.hasColorBg)
				{
					...
					bgcolorold.a *= adapter.par_e.HIGHLIGHTER_COLOR_OPACITY;

					HL_MOD.DRAW_BGTEXTURE_OLD(bgrect, bgcolorold);
			...
			{
				HL_MOD.autoMod.__FilterCacheClear();
			...
			void DrawLine(int index, Rect lineRect)
			{
				...
				var filter = HighLighterCommonData.GetColorFilters[index];
				...
				//	if (adapter.IS_HIERARCHY())     //FILTER COMPS
					...
					else*/
					{
						...
						_R_.height = 20;
						var new_hasColorText = HighLighterStyle.TOGGLE_LEFT(_R_, "<i>Label</i>:", filter.hasColorText);
						...
						_R_.x = seg.x + PPP;
						_R_.height = 20;
						var new_hasColorBg = HighLighterStyle.TOGGLE_LEFT(_R_, "<i>BG</i>   :", filter.hasColorBg);
						...
						HighLighterStyle.LABEL(_R_, "<i>Icon:</i>");
						var IR = _R_;
						...
						var fon = GUI.enabled;
						GUI.enabled = newicon;
						var new_hasColorIcon = HighLighterStyle.TOGGLE_LEFT(_R_, "<i>Icon Color</i>:", filter.hasColorIcon);
