...
namespace EMX.HierarchyPlugin.Editor.Windows
{
	public partial class HighlighterWindow : Windows.IWindow
	{
		...
		float DrawHiglighter(Rect inputrect)
		{
			...
			HighLighterStyle.LABEL(inputrect, "<i>Highlighter:</i>");
...

			{
				...
				if (!(HighlighterCache_Icons.GET_CONTENT(source).add_icon || Enable_BackgroundColor || LabelEnable))
				{
					rad.width *= 3;
					HighLighterStyle.LABEL(rad, "<i>Use ctrl click to close the window immediately</i>");
				}
				...
				if ((HighlighterCache_Icons.GET_CONTENT(source).add_icon || Enable_BackgroundColor || LabelEnable) && GUI.Button(rad, "Clear", adapter.STYLE_DEFBUTTON_right))
				{
				...
				/*   var newchild = adapter.TOGGLE_LEFT(rad, "<i>Apply To</i> <b>Child</b>", getcolor.child);
			...
			//* HIGLIGHTER */
			{
				...
				//if (en)
				{
					...
					if (Enable_BackgroundColor)
					{
						...
						RECT = r;
						HighLighterStyle.LABEL(RECT, "<i>Horizontal Align:</i>");
						r.y += r.height;


						RECT = r;
						/* LABEL(RECT, "<i>Left Align:</i>");
						 RECT.x += RECT.width / 5;
