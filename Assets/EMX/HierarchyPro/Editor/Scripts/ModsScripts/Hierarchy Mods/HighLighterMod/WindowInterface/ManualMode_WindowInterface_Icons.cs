...
namespace EMX.HierarchyPlugin.Editor.Windows
{
	public partial class HighlighterWindow : Windows.IWindow
	{
		...
		void DRAWICON(Rect inputrect)
		{
			...
			var im = HighlighterCache_Icons.GET_CONTENT(source).add_icon;
			if (im != null)
			{
				GUI.DrawTexture(R_CLOSE, adapter.GetExternalModOld(  "HIPERUI_CLOSE"));
				if (Button(R_CLOSE, ""))
				{
				...
				HighlighterCache_Icons.CheckIconIfAlreadyHas(source, im);
			}
		...
		{   /*  inputrect.x += 2;
          ...
          inputrect.height -= 4;*/
			...
			{   // GUI.BeginVertical();//label
				...
				adapter.INTERNAL_BOX(new Rect(est, GROUP_RECT.y, 1, inputrect.height));
