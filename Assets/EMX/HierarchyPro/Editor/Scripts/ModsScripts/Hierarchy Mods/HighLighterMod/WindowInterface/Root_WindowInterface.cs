...
namespace EMX.HierarchyPlugin.Editor.Windows
{
	public partial class HighlighterWindow : Windows.IWindow
	{
		...
		internal static void Init(MousePos rect, string title, Action<Texture, string> _SetIconImage, Texture _GetTexture, Action<TempColorClass, string> _SetHiglightColor
								   ...
								   , Action<Color32, string> _SetIconColor, Func<Color32?> _GetIconColor, HierarchyObject _source, PluginInstance inadapter)
		{
			...
			repaintW = CURRENT_WINDOW = Windows.IWindow.private_Init(rect, typeof(HighlighterWindow), adapter, title, useAnim: true) as HighlighterWindow;
		...
		internal static HighlighterWindow CURRENT_WINDOW;
		...
		new static PluginInstance adapter = null;
		// static PluginInstance static_adapter { get { return adapter; } }
		static HighlighterWindow repaintW;
		...
		static internal GUIStyle label
		{
			get
			{
				if (__label == null) __label = Root.p[0].label;
				return __label;
			}
		}
		...
		static internal GUIStyle button
		{
			get
			{
				if (__button == null) __button = Root.p[0].button;
				return __button;
			}
		}
		...
		internal static void CHANGE_GUI(PluginInstance adapter)
		{
			if (changedgui)
			{
				RESTORE_GUI();
		...
		protected override void OnGUI()
		{
			...
			string[] cats = new[] { "MANUAL MODE", "AUTO MODE", "", "☰CompsPresets" };
			if (!adapter.par_e.USE_MANUAL_HIGHLIGHTER_MOD) { cats[0] = ""; colors[0] = Color.clear; }
			if (!adapter.par_e.USE_AUTO_HIGHLIGHTER_MOD) { cats[1] = ""; colors[1] = Color.clear; }
			if (!adapter.par_e.USE_CUSTOM_PRESETS_MOD) { cats[3] = ""; colors[3] = Color.clear; }
			...
			if (nv == 0)     //   var PRESET_STRING = "Highlighter";
			{
				...
				FULL_RECT.height = singleLineHeight;
				HighLighterStyle.LABEL(FULL_RECT, "<i>Icon:</i>");
