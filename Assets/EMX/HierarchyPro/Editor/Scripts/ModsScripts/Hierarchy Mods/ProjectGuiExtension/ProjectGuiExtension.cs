    ...
    internal class ProjectGuiExtension
    {
        ...
        void asdasd(string s, Rect rect)
        {
            ...
            if (lastEvent != Event.current.type)
            {
                ...
                GUIInit = true;
                Window.AssignInstance(null, ref window, p.ProjectBrowserWindowType); // WINDOW INIT
...
#pragma warning restore
        ...
        bool INIT_TREE()
        {
            ...
            {
                onGUIRowCallback = p.m_TreeViewFieldInfo.FieldType.GetProperty("onGUIRowCallback", (BindingFlags)(-1)) ?? throw new Exception("Project onGUIRowCallback");
                if (onGUIRowCallback == null) return false;
            }

            Window.AssignInstance(p, ref p_w, p.ProjectBrowserWindowType);
            var w = p_w.Instance;
            //  var w = window();
            //  if (w == null)
            //  {
            ...
            var tree = p.m_TreeViewFieldInfoForProject(w).GetValue(w);
            // var tree = p.GetTreeViewontroller(w);// m_TreeView(w);
        ...
        void proj_Additional(int id, Rect rect)
        {
