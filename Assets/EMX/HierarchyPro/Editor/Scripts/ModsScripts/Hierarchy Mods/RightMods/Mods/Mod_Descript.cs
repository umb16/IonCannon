 ...
 #pragma warning restore*/
		...
		internal void CREATE_NEW_ESCRIPTION(PluginInstance adapter, MousePos pos, HierarchyObject _o, bool singleObject) //  var o = _o.go;
		{
			...
			Action<string> act = (str) =>
			{
				...
				HierarchyTempSceneDataGetter.SetUndo("Change description");
				...
				//  try
				{
					...
					if (singleObject || adapter.ha.SELECTED_GAMEOBJECTS().All(selO => selO != _o))
					{
			...
			Windows.InputWindow.Init(pos, "New description", adapter, act, act, textInputSet: input);
		}
