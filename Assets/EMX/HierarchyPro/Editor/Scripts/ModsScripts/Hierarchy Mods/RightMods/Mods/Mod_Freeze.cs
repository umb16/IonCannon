	...
	internal class Mod_Freeze : RightModBaseClass
	{
		...
		void onSelectionChange()
		{
			...
			if (adapter.par_e.RIGHT_FREEZE_LOCK_SCENE_VIEW)
			{
				foreach (var gameObject in so)
				{
					...
					{
						if (UNDO.wasUndo && !BookmarksforGameObjectsModInstance.ignoreLock.Contains(gameObject.go)) ArrayUtility.Add(ref BookmarksforGameObjectsModInstance.ignoreLock, gameObject.go);
						if (!BookmarksforGameObjectsModInstance.ignoreLock.Contains(gameObject.go))
						{
							wasShanged = true;
						}
					}
			...
			if (wasShanged)
			{
				...
				var so2 = lastSel.Where(l => l && !BookmarksforGameObjectsModInstance.ignoreLock.Any(i => i && i.GetInstanceID() == l.GetInstanceID())).ToArray();
				Selection.objects = so2;
				adapter.ha.InternalClearSelection(so2.Select(l => l ? l.GetInstanceID() : -1).ToArray());
			}
			if (BookmarksforGameObjectsModInstance.ignoreLock.Length != 0) BookmarksforGameObjectsModInstance.ignoreLock = new GameObject[0];
		}
		...
		void BUTTON_ACTION(Rect worldOffset, Rect inputRect, DrawStackMethodsWrapperData data, HierarchyObject _o)
		{
			if (EVENT.button == 1)
			{
				...
				Windows.SearchWindow.Init(mp, SearchHelper, "All", CallHeader(), this, adapter, _o);
				// EditorGUIUtility.ic
			}
		}
