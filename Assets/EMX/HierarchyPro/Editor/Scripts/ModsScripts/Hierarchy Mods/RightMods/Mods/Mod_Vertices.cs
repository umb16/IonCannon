...
namespace EMX.HierarchyPlugin.Editor.Mods
{
    ...
    internal class Mod_Vertices : RightModBaseClass
    {
        ...
        string BuildFullName( Mod_VerticesHelper des )
        {
            return BuildFullName( adapter.par_e.RIGHT_MOD_VERTICES_SCAN_TYPE, des );
        }

        string BuildFullName( VerticesModuleTypeEnum type, Mod_VerticesHelper des )
        {
            return HeaderText + " - " + nameRight( type, des );
        }

        string nameRight( VerticesModuleTypeEnum type, Mod_VerticesHelper des )
        {
            switch ( type )
            {
...
#pragma warning restore
        ...
        static void StartBroadcasting( )
        {
            ...
            currentList = Tools.AllSceneObjectsInterator( Root.p[ 0 ] ).GetEnumerator();
            //currentList.Reverse();
        ...
        internal void CalcBroadCast( ) // var s = EditorSceneManager.GetActiveScene();
        {
            ...
            //while (currentIndex < currentList.Count)
            {
                ...
                if ( interator > Root.p[ 0 ].par_e.RIGHT_MOD_BROADCASTING_PREFOMANCE ) break;
            }
                    ...
                    if ( Math.Abs( EditorApplication.timeSinceStartup - dsc.updateTimer[ adapter.o.id ] ) > 0.5 )
                    {
                        ...
                        if ( Allow( o ) )
                        {
                            ...
                            if ( old != dsc.broadCastValue[ adapter.o.id ].memory )
                            {
                                ...
                                //  adapter.RESET_DRAW_STACKS();
                                if ( adapter.firstFrame < 4 ) ResetStack();
                                //adapter.RESET_DRAW_STACKS(); ///////////////////////
                                else ResetStack();
...
#pragma warning restore
            ...
            if ( EVENT.button == adapter.MOUSE_BUTTON_0)
            {
                ...
                menu.AddItem( new GUIContent( "Use Broadcast To Children" ), adapter.par_e.RIGHT_MOD_BROADCAST_ENABLED, ( ) =>
                   {
                       Clear();
            ...
            if ( EVENT.button == adapter.MOUSE_BUTTON_1 && content.text != "error" )
            {
                ...
                else
                {
                    if ( !dsc.BroadcastingInitializeAllObjects ) // var pos = InputData.WidnwoRect(!callFromExternal(), EVENT.mousePosition, 128, 68, adapter );
                    {
                        ...
                        Windows.InputWindow.Init( pos, "", adapter, null, null, "Waiting for refresh..." );
                    }
                    ...
                    else
                    {
                        ...
                        { // InputData.Init(pos, "", null, null, "hasn't own value");
                            var pos = new MousePos(EVENT.mousePosition, MousePos.Type.Input_128_68, !callFromExternal(), adapter);
                            Windows.InputWindow.Init( pos, "", adapter, null, null, "Hasn't value, click to one of the children" );
                        }
        ...
        internal override Windows.SearchWindow.FillterData_Inputs CallHeader( ) //  var s = EditorSceneManager.GetActiveScene();
        {
            ...
            {
                var pos = new MousePos(EVENT.mousePosition, MousePos.Type.Input_128_68, !callFromExternal(), adapter);
                Windows.InputWindow.Init( pos, "", adapter, null, null, "wait for refresh" );
                return null;
            }
