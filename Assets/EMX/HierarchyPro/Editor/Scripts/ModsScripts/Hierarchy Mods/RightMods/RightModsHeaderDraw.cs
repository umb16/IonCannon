			...
			for (int i = 0; i < p.window.modsCount; i++)
			{
					 ...
					 p.window.modulesPosesX[p.window.tempModsData[i].i_targetMod] != firstDrawRect.x)
				{
					...
					//  if ( p.EVENT.type == EventType.Repaint )
					{
						...
						///v.x = width - v.x;
						//WR.x = width - WR.x;
						if (p.EVENT.type == EventType.Repaint) vx = Mathf.MoveTowards(vx, WR.x, p.deltaTime * 500.6f);
						bool rep = false;
						if (vx != WR.x) rep = true;
						//WR.x = width - WR.x;
						//v.x = width - v.x;
						...
						WR.x = vx;
						firstDrawRect = WR;
						if (rep && (p.EVENT.type == EventType.Repaint)) p.RepaintWindow(true);
						...
						p.window.mouseEventDrag = (b, deltaTime) =>
						 {
							 ...
							 float DIF = -(int)startMouse.x + (int)GUIUtility.GUIToScreenPoint(p.EVENT.mousePosition).x;
							 ...
							 //   var upnext = p.window.tempModsData[p.window.captured_mod_arr].sibling;
							 //  var downnext = p.window.tempModsData[p.window.captured_mod_arr].sibling;
							 TempModData? nextMod = null;
							 const float M = 0.6f;
							 bool move_left, move_right;
							 if (move_right = (p.window.captured_mod_arr != 0 && DIF > +p.window.tempModsData[p.window.captured_mod_arr - 1].width * M))
								 nextMod = p.window.tempModsData[p.window.captured_mod_arr - 1];
							 if (move_left = (
									p.window.captured_mod_arr < p.window.tempModsData.Length - 1
									&& p.window.tempModsData[p.window.captured_mod_arr + 1].enabled
									&& DIF < -p.window.tempModsData[p.window.captured_mod_arr + 1].width * M))
								 nextMod = p.window.tempModsData[p.window.captured_mod_arr + 1];
							 // Debug.Log( DIF + " " + p.window.captured_mod_arr + " " + p.window.tempModsData[ p.window.captured_mod_arr + 1 ].width );
							 /*Debug.Log( DIF + " " +  (-p.window.tempModsData[ p.window.captured_mod_arr + 1 ].width * M) + " " + 
									   p.window.tempModsData[ p.window.captured_mod_arr - 1 ].width * M );*/
							 if (nextMod.HasValue)
							 {

								 var o = nextMod.Value.targetMod.savedData.sibling;
								 nextMod.Value.targetMod.savedData.sibling = p.window.tempModsData[p.window.captured_mod_arr].targetMod.savedData.sibling;
								 p.window.tempModsData[p.window.captured_mod_arr].targetMod.savedData.sibling = o;
								 // var increase = nextMod.Value.sibling >  p.window.tempModsData[p.window.captured_mod_arr].sibling;
								 if (move_left)
								 {
									 /// Debug.Log( "ASD" );
									 ...
									 p.window.captured_mod_arr++;
								 }
								 if (move_right)
								 {
									 /// startRect.x += p.window.tempModsData[ p.window.captured_mod_arr ].width;
									 startMouse.x += nextMod.Value.width;
									 startX += nextMod.Value.width;
									 p.window.captured_mod_arr--;
								 }
							 ...
							 {
								 if (startMouse == GUIUtility.GUIToScreenPoint(p.EVENT.mousePosition))
								 {
				...
				//RESIZE AREA;
				{
					...
					//if (GUI.Button(drawRect, "") && p.EVENT.button == 0)
					{
						...
						p.window.mouseEvent = (b, deltaTime) =>
						 {
							 var dif = (int)startMouse.x - (int)GUIUtility.GUIToScreenPoint(p.EVENT.mousePosition).x;
							 p.RESET_DRAWSTACK();
				...
				//MENU OPEN
				{
					...
					if (modRect.Contains(p.EVENT.mousePosition) && p.EVENT.type == EventType.MouseDown && p.EVENT.button == p.MOUSE_BUTTON_1 && p.window.mouseEventDrag == null && p.window.mouseEvent == null)
					{
						...
						if (!p.window.rightModIndexAndOnMouseUp.ContainsKey(captureModule.savedData.sibling))
						{
							...
							p.PUSH_ONMOUSEUP(p.window.EVENT_HELPER_ONUP, p.window.Instance);
							...
							p.window.rightModIndexAndOnMouseUp.Add(captureModule.savedData.sibling, () =>
							{
								if (captureRect.Contains(p.EVENT.mousePosition))
								{
									...
									if (!error)
									{
										...
										if (result != null)
										{
											var mp = new MousePos(p.EVENT.mousePosition, MousePos.Type.Search_356_0, true, p);
											Windows.SearchWindow.Init(
																				mp, capturedModule.SearchHelper, "All", /*topGui*/result, capturedModule, p, null);
										}
									}
								}
							});
