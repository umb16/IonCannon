...
namespace EMX.HierarchyPlugin.Editor.Mods
{
  ...
  #pragma warning restore
		...
		internal void FirstFrameOnGUI()
		{
			...
			adapter.hierarchyModification.prebapButtonStyle.margin.right = adapter.par_e.SET_ACTIVE_PREFAB_BUTTON_OFFSET ? (SMALL ? (rawIW) : (rawIW - 3)) : 0;
			///bgrect.width += 40;
			///a
			///
			...
			if (adapter.ha.hasShowingPrefabHeader /*&& adapter.o.HasBrefabButton*/)
				if (prefabOffset)
					bgrect.x += adapter.hierarchyModification.PREFAB_BUTTON_SIZE - 2;
			...
			bgrect.width += adapter.hierarchyModification.PREFAB_BUTTON_SIZE;
					...
					if (adapter.EVENT.button == 1)
					{
						if (!objectIsHiddenAndLock)
						{
							if (SceneView.lastActiveSceneView == null) //var pos = InputData.WidnwoRect(!callFromExternal(), adapter.EVENT.mousePosition, 128, 68, adapter );
							{
								...
								Windows.InputWindow.Init(pos, "", adapter, null, null, "Please open 'SceneView' window");
							}
