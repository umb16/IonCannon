...
namespace EMX.HierarchyPlugin.Editor
{
	class RightClickOnGameObjectMenuRegistrator
	{
		...
		internal static void ContextMenu(GenericMenu menu, GameObject go)
		{
			...
			menu.AddItem(new GUIContent("" + Root.PN + "/Select Main Settings"), false, () =>
			{
				Settings.MainSettingsEnabler_Window.Select<Settings.MainSettingsEnabler_Window>();
			});
			menu.AddSeparator("" + Root.PN + "/");
			menu.AddItem(new GUIContent("" + Root.PN + "/Select RightClick Menu Settings"), false, () =>
			{
			...
			if (externalMod_MenuItems.Count != 0)
			{
				...
				{
					var r = item.release;
					menu.AddItem(new GUIContent("" + Root.PN + "/" + item.path), false, () => { r(0, item.text); ; });
				}
			}
		}
		...
		{
			externalMod_MenuItems = _externalMod_MenuItems.OrderBy(b => b.priority).ToList();
		}
		...
		internal static void SubscribeEvents(EditorSubscriber sbs)
		{
			...
			sbs.BuildedOnGUI_first += HierarchyWindowEvent;
		}
		...
		static Dictionary<string, KeyCode> avaliableKeys = new Dictionary<string, KeyCode>()
	{
		...
		static bool GenerateCustomMenu(GenericMenu menu, GameObject tapedGO)
		{
			...
			bool wasRoot = false;
			if (ExtensionInterface_RightClickOnGameObjectMenuItem.ItemsPlacementFolder == "" || sorted.Any(c => c.Name.StartsWith(ExtensionInterface_RightClickOnGameObjectMenuItem.ItemsPlacementFolder, StringComparison.OrdinalIgnoreCase)))
			{
				wasRoot = true;
				menu.AddSeparator("");
			}
...
namespace EMX.CustomizationHierarchy
{
	...
	public abstract class ExtensionInterface_RightClickOnGameObjectMenuItem
	{
		...
		public static string ItemsPlacementFolder { get { return Root.p[0].par_e.RCGO_MENU_PLACE_TO_HIERFOLDER ? "" + Root.PN + "/" : ""; } }
		...
		public ExtensionInterface_RightClickOnGameObjectMenuItem()
		{
			...
			{
				Debug.LogWarning("Warning! You already have a '" + GetType().Name + "' item. Please check the 'SpikeNailRegistrationToImproveInitPerfomance()' method");
			}
