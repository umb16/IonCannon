...
namespace EMX.HierarchyPlugin.Editor.Mods
{
	...
	class SnapMod
	{
		...
		static string SnapModSwitcherDir { get { return Folders.PluginInternalFolder + "/Editor/_SAVED_DATA/"; } }
...
namespace EMX." + Root.PN_NS + @".Editor.Mods
{
    ...
    public partial class SnapButtonsForInspectorGUI : UnityEditor.Editor
    {
		...
		public static CachedPref SNAP_SNAP_USEHOTKEYS = new CachedPref("SNAP_SNAP_USE", true) { name = "HotKey Enable/Disable Snap" };
		public static CachedPref SNAP_SNAP_HOTKEYS = new CachedPref("SNAP_SNAP_HOTKEYS", (int)KeyCode.K) { name = "Ctrl+Shift+..." };

		public static CachedPref SNAP_SURFACE_USEHOTKEYS = new CachedPref("SNAP_SURFACE_USE", true) { name = "HotKey Enable/Disable Surface" };
		public static CachedPref SNAP_SURFACE_HOTKEYS = new CachedPref("SNAP_SURFACE_HOTKEYS", (int)KeyCode.L) { name = "Ctrl+Shift+..." };

		public static CachedPref SNAP_FASTINVERT_USEHOTKEYS = new CachedPref("SNAP_INVERT_USE", false) { name = "Holding HotKey to fast invert Snap" };
		...
		public static CachedPref SNAP_AUTOAPPLY = new CachedPref("SNAP_TOOGLES_SNAP_AUTOAPPLY", false) { name = "Auto-Apply Snap (If the selection changes)" };
		...
		public static CachedPref PLACE_ON_SURFACE_BOUNDS = new CachedPref("SNAP_TOOGLES_PLACE_ON_SURFACE_BOUNDS", true) { name = "Snap To Pivot" };
		...
		public static CachedPref ALIGN_UP_VECTOR = new CachedPref("SNAP_TOOGLES_ALIGN_UP_VECTOR", 0) { name = "Auto-Apply Snap" };
		...
		static void SET_DIRTY(Transform t)
		{
			...
			Root.p[0].MarkSceneDirty(t.gameObject.scene);
		}
		...
		void DO_SURFACE_BUTTON(Rect r)
		{
			...
			if (GUI.Button(r, raycastContent, buttonStyle))
			{
				...
				else
				{
					...
					menu.AddSeparator("");
					menu.AddItem(new GUIContent(SnapMod.PLACE_ON_SURFACE_BOUNDS.name), !SnapMod.PLACE_ON_SURFACE_BOUNDS, () => SnapMod.PLACE_ON_SURFACE_BOUNDS.Set(!SnapMod.PLACE_ON_SURFACE_BOUNDS));
					menu.AddSeparator("");
					menu.AddItem(new GUIContent("Open Plugin Snap Settings"), false, () => { Settings.MainSettingsEnabler_Window.Select<Settings.ST_Window>(); });
					menu.ShowAsContext();
				}
	...
	class CachedPref
	{
		...
		bool m_boolValue
		{
			...
			set { Root.p[0].par_e.SET(m_registryKey, (m_boolCache = value).Value); }
		}
		...
		int m_intValue
		{
			...
			set { Root.p[0].par_e.SET(m_registryKey, (m_intCache = value).Value); }
		}
	}
}
