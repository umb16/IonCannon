	...
	internal class DrawStack
	{
		...
		*/
		...
		public DrawStackItem Draw_ModuleButton(Rect rect, GUIContent content, DrawStackMethodsWrapper action, bool hasContent, object args, bool useContentForButton, GUIStyle style,
			bool USE_GO = false, bool callFromExternal = false)
		{
		...
		public DrawStackItem Draw_SimpleButton(Rect rect, GUIContent content, DrawStackMethodsWrapper action, object args, bool useContentForButton, GUIStyle style, bool USE_GO = false, bool callFromExternal = false)
		{
				...
				case ModulesDrawType.GUIDrawTextureWithBordersAndMaterial:
					{
						...
						c *= GUI.color;
						gl._DrawTexture(stack.rect(worldOffset), stack.texture, ref c, stack.borders, adapter.par_e.HIghlighterExternalMaterial, ref stack.clipRect);
						break;
					}
				...
				case ModulesDrawType.GUIDrawTextureWithBorders:
					{
						...
						gl._DrawTexture(stack.rect(worldOffset), stack.texture, ref c, stack.borders, ref stack.clipRect);
						break;
					}
				...
				case ModulesDrawType.GUIDrawTexture:
					{
						...
						gl._DrawTexture(stack.rect(worldOffset), stack.texture, ref c);
						break;
					}
				...
				case ModulesDrawType.ModuleButton: //stack.self_SkipContentInstance ? null : stack.self_ContentInstance
					{
						...
						if (stack.UseGameObjectEnable)
						{
							...
							if ((stack.content != null && !string.IsNullOrEmpty(stack.content.text) || stack.callFromExternal || !adapter.hashoveredItem || adapter.hoverID == _o.id) &&
								adapter.ModuleButton(stack.rect(worldOffset), stack.content ?? emptyContent, stack.hasContent, stack.style))
							{
								data.args = stack.args;
...
#pragma warning restore
		...
		internal void END_DRAW(HierarchyObject _o)
		{
		...
		internal void Draw_Label(Rect rect, GUIContent content, GUIStyle style, bool USE_GO, Color? color = null, bool? ADDITIONAL_ENABLE = null)
		{
			...
			else emptyStack.DrawSIngleItem(emptyStack.Draw_Label(rect, content, style, USE_GO, color, ADDITIONAL_ENABLE));
		}
		...
		{ //if ( !adapter.DRAW_LABEL_FOR_EMPTY_CONTENT && content == "-" ) return;
			...
			else emptyStack.DrawSIngleItem(emptyStack.Draw_LabelWithTextColor(rect, content, textColor, style, USE_GO, color, ADDITIONAL_ENABLE));
		}
			...
			bool? ADDITIONAL_ENABLE = null, int SWITCHER = 0) //if ( !adapter.DRAW_LABEL_FOR_EMPTY_CONTENT && content == "-" ) return;
		{
			...
			else emptyStack.DrawSIngleItem(emptyStack.Draw_LabelWithTextColor(rect, content, textColor, style, USE_GO, color, ADDITIONAL_ENABLE).ADD_SWITCHER(SWITCHER));
		}
		...
		internal void Draw_ModuleButton(Rect rect, GUIContent content, DrawStackMethodsWrapper method, bool hasContent, object args = null, bool useContentForButton = false,
			...
			bool USE_GO = false)
		{
			if (PERFOMANCE_BARS && perfadditionalcondition) CURRENT_STACK.Draw_ModuleButton(rect, content, method, hasContent, args, useContentForButton, style, USE_GO, callFromExternal());
			else if (PERFOMANCE_BARS) emptyStack.DrawSIngleItem(emptyStack.Draw_ModuleButton(rect, content, method, hasContent, args, useContentForButton, style, USE_GO, callFromExternal()));
			else emptyStack.DrawSIngleItem(emptyStack.Draw_SimpleButton(rect, content, method, args, useContentForButton, style, USE_GO, callFromExternal()));
		}
			...
			bool USE_GO = false)
		{
			...
			else emptyStack.DrawSIngleItem(emptyStack.Draw_SimpleButton(rect, content, method, args, useContentForButton, style, USE_GO));
		}
