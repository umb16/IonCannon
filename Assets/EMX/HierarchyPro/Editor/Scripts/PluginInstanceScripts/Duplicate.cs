...
namespace EMX.HierarchyPlugin.Editor
{
	...
	class DuplicateHelper
	{
					...
					if (dh)
					{
						...
						//  if ( !string.IsNullOrEmpty( dh.cachedData ) )
						{
							...
							foreach (var s1 in data)
							{
								...
								var key = int.Parse(k);
								var m = Root.p[pluginID].modsController.saveModsInterator[key];
		...
		internal List<HierarchyObject> AddDataToName(HierarchyObject[] transs, bool writeUndo)
		{
			...
			if (Root.p[pluginID].DisabledSavedData()) return result;
			StringFlush dh = null;
			string s = null;
			foreach (var o in transs)
			{
				...
				for (int i = 0, L = Root.p[pluginID].modsController.saveModsInterator.Count; i < L; i++)
				{
					var m = Root.p[pluginID].modsController.saveModsInterator[i];
		...
		{
			if (pluginID == 1) return Root.p[pluginID].ha.SELECTED_OBJECTS(); //SpeedUp

			var result = new List<HierarchyObject>();
			var sel = Root.p[pluginID].ha.SELECTED_OBJECTS();
