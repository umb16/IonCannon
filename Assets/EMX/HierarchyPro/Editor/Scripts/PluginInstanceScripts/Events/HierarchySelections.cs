	...
	partial class HierarchyActions //HierarchySelections
	{
		...
		internal HierarchyObject[] FILTER_GAMEOBJECTS(UnityEngine.Object[] o)
		{
			if (pluginID == 0) return /* Selection.gameObjects == null ? new GameObject[0] :*/
				  o.Select(g => g as GameObject).Where(g => g && isSceneObject(g)).Select(Cache.GetHierarchyObjectByInstanceID).ToArray();
			return o.Where(ob => ob && !string.IsNullOrEmpty(isProjectObject(ob))).Select(ob => Cache.GetHierarchyObjectByGUID(ob)).ToArray();
		}
		...
		internal HierarchyObject activeGameObject()
		{
			...
			return Cache.GetHierarchyObjectByGUID(Selection.activeObject);
		}
		...
		{
			Root.p[pluginID].HoverDisabled = true;
			Root.p[pluginID].hoverID = -1;
		}
		internal void internal_DisableHover()
		{
			if (!Root.p[pluginID].hashoveredItem) return;
			Root.p[pluginID].hoveredItem.SetValue(Root.p[pluginID].TreeController_current, null, null);
			Root.p[pluginID].hoverID = -1;
		}
		...
		internal void InternalClearSelection(int[] ids)
		{
			...
			var currentTree = Root.p[pluginID].GetTreeViewontroller(window);
			if (currentTree == null) return;

			var currentState = Root.p[pluginID]._state.GetValue(currentTree, null);
