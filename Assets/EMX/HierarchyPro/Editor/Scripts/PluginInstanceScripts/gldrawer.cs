...
namespace EMX.HierarchyPlugin.Editor
{
	...
	class GlDrawer
	{
					...
					for (int x = 0, L2 = S.glStack.Count; x < L2; x++)
					{
						...
						if (x == 0)
						{
							if (!R.mat)
							{
								...
								ClearStacks();
								Root.p[0].RESET_DRAWSTACK();
								Root.p[0].RepaintWindow(true);
								return;
							}
						}
						/*if (i == 0)
						...
						for (int z = 0; z < R.glCount; z++)
						{
							...
							if (R.glStack[z].border <= 0 || !t) drawer.draw_simple_quad_fast(ref R.glStack[z].rect, ref R.glStack[z].col, R.glStack[z].tex);
		...
		{
			if (!_defaultMaterial)
				_defaultMaterial = new Material(p.par_e.DEFAULT_SHADER_SHADER.ExternalMaterialReference);
		...
		static Material additionalMaterial()
		{
			if (!_additionalMaterial)
				_additionalMaterial = new Material(Root.p[0].par_e.DEFAULT_SHADER_SHADER.ExternalMaterialReference);
			return _additionalMaterial;
		}
		Vector3 tv3;
		void draw_simple_quad(ref Rect rect, ref Color c, IconData tex = null)
		{
		...
		void draw_simple_quad_fast(ref Rect rect, ref Color c, IconData tex = null)
		{
			if (rect.width <= 0 || rect.height <= 0) return;
			GL.Color(c);
		...
		void __DrawTexture(ref Rect rect, IconData tex, ref Color col, float border, Material mat, ref Rect? clipRect, bool fast = false)
		{
			...
			if (border == 0)
			{
				defaultMaterial().SetTexture(_MainTex, tex.texture);
				defaultMaterial().SetPass(0);
				draw_simple_quad(ref rect, ref col, tex);
			}
			else
			{
		...
		class GLLabelStack
		{
			...
			internal void _put_stack(Rect rect, GUIContent self_ContentInstance, TextAnchor alignment,
			 int fontSize,
			 Font font,
			  Color textColor,
		...
		internal void _DrawTexture(Rect rect, IconData tex, ref Color col, float border, ref Rect? clipRect )
		{
			if (p.EVENT.type != EventType.Repaint || rect.width <= 0 || rect.height <= 0) return;
		...
		{
			if (p.EVENT.type != EventType.Repaint || rect.width <= 0 || rect.height <= 0) return;
			if (!BAKE_DRAWER) __DrawTexture(ref rect, tex, ref col, border, mat, ref nullClip);
			else _put_stack(ref rect, tex, ref col, border, mat ?? defaultMaterial(), ref nullClip);
		}
		internal void _DrawTexture(Rect rect, IconData tex, ref Color col, float border, Material mat, ref Rect? clipRect)
		{
			if (p.EVENT.type != EventType.Repaint || rect.width <= 0 || rect.height <= 0) return;
			if (!BAKE_DRAWER) __DrawTexture(ref rect, tex, ref col, border, mat, ref clipRect);
		...
		internal void _DrawLabel(Rect rect, GUIContent self_ContentInstance, GUIStyle style, TextClipping clip = TextClipping.Clip)
		{
			...
			{
				drawCalls[dc].label_stack._put_stack(rect, self_ContentInstance, style.alignment
				, style.fontSize
				, style.font
