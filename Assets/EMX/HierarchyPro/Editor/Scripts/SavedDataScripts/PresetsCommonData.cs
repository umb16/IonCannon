...
namespace EMX.HierarchyPlugin.Editor
{
	...
	partial class PresetsCommonData : ScriptableObject
	{
		...
		const string TypeName = "PresetsCommonData.asset";
	...
	public class PresetsData
	{
		...
		public int Count => singlecomp_categories.categories.Length + fullhierarrchy_categories.categories.Length;
	...
	{
		[SerializeField] public PRESET_TYPE SetType;
		[SerializeField] public PresetSet[] categories;
		public string Save(int v)
		{
			StringBuilder sb = new StringBuilder();
			sb.AppendLine(((int)SetType).ToString());
			if (categories != null) for (int i = 0; i < categories.Length; i++) sb.AppendLine(categories[i].Save(v));
			return Convert.ToBase64String(Encoding.UTF8.GetBytes(sb.ToString()));
		}
	...
	public class PresetSet
	{
		...
		[SerializeField] public SetItem[] presets;
		...
		public string Save(int v)
		{
			...
			if (presets != null) for (int i = 0; i < presets.Length; i++) presets[i].Save(ref sb);
			return Convert.ToBase64String(Encoding.UTF8.GetBytes(sb.ToString()));
		}
		public void Load(string b64, int v)
		{
	...
	public class SetItem
	{
		...
		[SerializeField] public string external_heap_guid_copy;
		[SerializeField] public string json;

		public void Save(ref StringBuilder sb)
		{
			sb.AppendLine(" ");
			sb.AppendLine(name);
			sb.AppendLine(id_in_external_heap.ToString());
			sb.AppendLine(external_heap_guid_copy);
			sb.AppendLine(json);
		}
		public void Load(ref StringReader sr, int version)
		{
			name = sr.ReadLine();
			id_in_external_heap = int.Parse(sr.ReadLine());
			external_heap_guid_copy = sr.ReadLine();
			json = sr.ReadLine();
		}
	}
