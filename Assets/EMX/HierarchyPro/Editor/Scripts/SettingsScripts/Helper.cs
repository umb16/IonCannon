...
namespace EMX.HierarchyPlugin.Editor.Settings
{
	...
	class Draw
	{
		...
		static Dictionary<string, FIELD_SETTER> _setterCache = new Dictionary<string, FIELD_SETTER>();
		...
		static GUIStyle tb;
		internal static void TOOLBAR(string[] textArray, string KEY, float height = -1, object overrideObject = null, Rect? rect = null)
		{
			...
			for (int i = 0; i < textArray.Length; i++)
			{
				...
				if (GUI.Button(r, Draw.CONT(textArray[i]))) newb = i;
		...
		internal static GUIContent CONT(string t, string text)
		{
	...
	internal class CLASS_ENALBE
	{
		...
		internal dsp USE(string setter, bool inverce = false)
		{
			return USE(Draw.GetSetter(setter), inverce);
		}
		...
		internal dsp USE(string setter, float padding, bool inverce = false)
		{
			return USE(Draw.GetSetter(setter), padding, inverce);
		}
