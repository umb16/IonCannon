...
namespace EMX.HierarchyPlugin.Editor.Windows
{
	...
	public class IWindow : EditorWindow
	{
		...
		Events.MouseRawUp mouse_uo_helper;
		...
		{
#if UNITY_2018_3_OR_NEWER
			if (!(this is InputWindow || this is HighlighterWindow || this is SearchWindow)) return;
			EditorApplication.quitting -= _apquite;
			EditorApplication.quitting += _apquite;
			inst_t = this;
#endif
		...
		internal static IWindow private_Init(MousePos inrect, Type type, PluginInstance adapter, string title = "", Vector2? minSize = null, Vector2? maxSize = null, bool utils = false,
			bool useAnim = true /*, bool skipPositionAssign = false*/)
		{
			...
			w.mouse_uo_helper = new Events.MouseRawUp(adapter);
			...
			if (adapter.par_e.ENABLE_CUSTOMWINDOWS_OPENANIMATION && useAnim) w.Screenrect.height = 25;
			w.adapter = adapter;
...
#pragma warning restore
		...
		internal static float MAX_HEIGHT(PluginInstance a) //if (! a.window()) Debug.Log("ASD");
		{
			return a.window.position.height + 20;
			//Screen.currentResolution.height * (HEIGH_CLAMPER)
		}

		internal void SET_NEW_HEIGHT(PluginInstance adapter, float height)
		{
			...
			{
				if (targetRect.y + targetRect.height > adapter.window.Instance.position.y + MAX_HEIGHT(adapter))
					targetRect.y -= (targetRect.y + targetRect.height) -
									(adapter.window.Instance.position.y + MAX_HEIGHT(adapter));
			}
