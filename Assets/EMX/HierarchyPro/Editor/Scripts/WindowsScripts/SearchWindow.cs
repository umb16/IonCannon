		...
		internal class FillterData_Inputs
		{
			internal FillterData_Inputs( FillterData_Inputs oldin )
			{
				...
				else
				{
					...
					this.analizeEnumerator = Tools.AllSceneObjectsInterator( Adapter ).GetEnumerator();
				}
			}
...
#pragma warning restore
		...
		internal static IWindow Init( MousePos guiMousePos, string title, string searchFilter, FillterData_Inputs IN, ISearchable callBackModule,
			PluginInstance adapter, HierarchyObject _o, string extension = null, bool utils = false, bool useAnim = true, bool? calledWithControlOld = null,
			UnityEngine.Object activeObject = null )
		{
			...
			utils |= Adapter.par_e.SEARCH_PIN_WIN_BYDEFAULT;
			...
			if ( adapter.par_e.BIND_SEARCH_TO_LEFT && guiMousePos.Clamp ) guiMousePos.ScreenMousePosition = new Vector2( adapter.window.position.x + 20, guiMousePos.ScreenMousePosition.y );
			guiMousePos.Width = adapter.par_e.ADDITIONA_SEARCH_WINDOWS_WIDTH;
			...
			if ( !_inputWindow ) //new
			{
				_inputWindow = IWindow.private_Init( guiMousePos, typeof( SearchWindow ), adapter, "Search " + searchFilter + " " + adapter.pluginname,
		...
		IList<HierarchyObject[]> objectsList
		{
			get
			{
				var _sort = Adapter.par_e.SEARCH_SORT_MODE_GET(m_callBackType.GetType().Name);
		...
		void OnUpdateCache( bool skipIfSkip = false ) //  if (IN.analizedObjects == null)
		{
			...
			if ( !skip )
			{
				...
				if ( !__calledWIthControl ) active = adapter.par_e.SEARCH_USE_ROOT_ONLY ? ___o != null ? ___o.rootSlow().GetHardLoadObjectSlow() : null : null;

				IN.analizeEnumerator = Tools.AllSceneObjectsInterator( adapter, activeParent: active, extension: ___extension ).GetEnumerator();
			}
		...
		{
			base.CloseThis();
			if ( adapter != null && adapter.window.Instance ) adapter.window.Instance.Focus();
		}
		...
		{
			selectedO.Clear();
			if ( Adapter.ha.activeGameObject() != null )
			{
				...
				var o = Adapter.ha.activeGameObject();
		...
		{
			return Mathf.RoundToInt( /*Adapter.HALFFACTOR_8() * */Adapter.par_e.USE_LINE_HEIGHT ? Adapter.par_e.LINE_HEIGHT : 16 );
		}
		...
		float scrollHeight
		{
			get { return Math.Min( /*MAX_HEIGHT( adapter )*/ position.height /*- up*/ - yPosSum, s__HierarchyPos.Count * ItemH() ); }
		}
		...
		void SELECT( HierarchyObject[] list, bool useDOuble )
		{
			...
			if ( adapter.pluginID == 0 ) target = list.Where( o => o.go ).Select( o => o.go ).ToArray();
			if ( adapter.pluginID == 1 ) target = list.Select( o => AssetDatabase.LoadMainAssetAtPath( o.project.assetPath ) ).Where( a => a ).ToArray();
		...
		static float LINE1
		{
			get { return 42 * Adapter.HALFFACTOR_8(); }
		}
		...
		protected override void OnGUI( )
		{
			...
			if ( m_PIN ) adapter.gl._DrawRect( ref rect, new Color( 0.6f, 0.3f, 0.1f, 0.2f ) );
			adapter.gl._DrawTexture( new Rect( rect.x, (rect.height - IS) / 2, IS, IS ), Adapter.GetOldIcon( m_PIN ? "AUTOHIDEPIN" : "AUTOHIDEFREE" ) );
			...
			if ( GUI.Button( rect, ss, S1 ) )
			{
				...
				{
					desc = !desc;
					Adapter.par_e.SEARCH_SORT_MODE_SET( m_callBackType.GetType().Name, (int)sort | (desc ? 0x10 : 0) );
				}
				else
				{
			...
			adapter.gl._DrawRect( ref rect,ref bgc );
			rect.width -= 8;
			...
			{ // if (scrollHeightOver) contentRect.width -= 15;
				...
				foreach ( var gameObjectse in objectsList )
				{
					for ( int j = 0 ; j < gameObjectse.Length ; j++ )
					{
						...
						if ( adapter.pluginID == 0 && !OBJECT.go ) continue;
						...
						if ( adapter.ha.IsSelected( OBJECT.id ) )
						{
							r = rect;
						...
						if ( Event.current.type == EventType.MouseDown && Event.current.button == 0 && rect.Contains( Event.current.mousePosition ) )
						{
							...
							var selection = Adapter.ha.SELECTED_GAMEOBJECTS();
						...
						{ // drawContent.text = OBJECT.ToString();
							...
							{ /* var ir = rect;
								...
								tempDynamicRect.Set( new Rect( label_rect.x, label_rect.y, contentRect.width + contentRect.x - label_rect.x, label_rect.height ),
								 ...
								 tempColor = adapter.modsController.highLighterMod.DrawBackground(null, null, tempDynamicRect, OBJECT, 1, resetFonts: false );
							}
						...
						if ( BUT )
						{
							...
							{
								var oldSel = (Adapter.ha.SELECTED_GAMEOBJECTS() ?? new HierarchyObject[0]).Where(o => drawIndex.ContainsKey(o.id)).ToList();
			...
			{ /* if (nores)
				...
				rect.y += rect.height;
				var n1 = EditorGUI.ToggleLeft(rect, "Include disabled objects", adapter.par_e.SEARCH_SHOW_DISABLED_OBJECT);
				rect.y += rect.height;
				var n2 = EditorGUI.ToggleLeft(rect, "Search uses only the root object of the clicked object", adapter.par_e.SEARCH_USE_ROOT_ONLY);
				if ( n1 != adapter.par_e.SEARCH_SHOW_DISABLED_OBJECT || n2 != adapter.par_e.SEARCH_USE_ROOT_ONLY )
				{
					adapter.par_e.SEARCH_USE_ROOT_ONLY = n2;
					adapter.par_e.SEARCH_SHOW_DISABLED_OBJECT = n1;
					// adapter.SavePrefs();
		...
		{
			get { return __tempDynamicRect ?? (__tempDynamicRect = new DynamicRect() ); }
		}
		...
		private void StartBroadcasting( )
		{
			...
			CountProgress = Tools.AllSceneObjectsInteratorCount( adapter );
...
#pragma warning restore
		...
		internal void CalcBroadCast( bool skipsize = false )
		{
			...
			bool start = false;
			double counter = 0;
			var dsbuse = !adapter.par_e.SEARCH_SHOW_DISABLED_OBJECT;
			var limit = adapter.pluginID == 0 ? 0.04f : 0.06f;
			while ( true )
			{
				if ( !start )
				{
				...
				if ( current != null )
				{
					...
					if ( adapter.pluginID == 1 && (current.go.hideFlags & Tools.SearchFlags) == Tools.SearchFlags )
					{
						currentIndex = Tools.AllSceneObjectsInteratorProgress();
						continue;
					}
