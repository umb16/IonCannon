...
namespace EMX.HierarchyPlugin.Editor
{
	public class HierarchyTempSceneData : MonoBehaviour
	{
		...
		public const HideFlags FLAGS = HideFlags.DontSave | HideFlags.DontSaveInBuild | HideFlags.DontSaveInEditor | HideFlags.HideInHierarchy;
		...
		public static HierarchyTempSceneData InstanceSlow(Scene s)
		{
			...
			if (l.GetComponent<HierarchyTempSceneData>()) return l.GetComponent<HierarchyTempSceneData>();
			return l.AddComponent<HierarchyTempSceneData>();
		}
		...
		public static void __(ref Scene scene, int t, bool skipLoad = false)
		{
					...
					if (!skipSiblings)
						for (int i = assigned.Length; i < result.Length; i++)
						{
							if (!result[i])
							{
								...
								var wrong = 0;
								var S = 0;
								if (siblings[S] >= tempRoots.Length) siblings[S] = Index(ref tempRoots, ref names[S]);
								if (siblings[S] == -1) break;
								if (tempRoots[S].name != names[S])
								{
									var s = Index(ref tempRoots, ref names[S]);
									if (s != -1) S = s;
									else wrong++;
								}
								currentRoot = tempRoots[S].transform;
								S++;
								while (S < siblings.Length)
								{
									if (siblings[S] >= currentRoot.childCount) siblings[S] = Index(currentRoot, ref names[S]);
									if (siblings[S] == -1) goto br;
									if (currentRoot.GetChild(S).name != names[S])
									{
										var s = Index(currentRoot, ref names[S]);
										if (s != -1) S = s;
										else wrong++;
									}
									currentRoot = currentRoot.GetChild(S);
								}
								if (wrong < 2 || currentRoot.name == names[names.Length])
			...
			foreach (var item in _T.tempSceneTypesList[t].objects)
			{
				...
				var id = item.target.GetInstanceID();
				if (!res.ContainsKey(id)) res.Add(id, item);
			}
			_GetObjectData[scene.GetHashCode()].Add(t, res);
		}

		static int Index(ref GameObject[] g, ref string s)
		{
			for (int i = 0; i < g.Length; i++) if (g[i].name == s) return i; return -1;
		}
		static int Index(Transform g, ref string s)
		{
			for (int i = 0; i < g.childCount; i++) if (g.GetChild(i).name == s) return i; return -1;
		}
		...
		public static void SaveToExternalFile(int t, Scene scene, bool skipCache = false)
		{
			...
			if (!skipCache)
			{
				...
				target = __target.Values.Where(g => g.target).ToArray();
			...
			else
			{
				...
				target = _T.tempSceneTypesList[t].objects.Where(g => g.target).ToArray();
			}
            ...
            {
                if (!item.target) Debug.Log(item.stringValue);
            }
#endif
			var savedId = target.Select(g => { return getSavedGlobalId(g.target); }).ToArray();
			...
			HierarchyExternalSceneData.WriteObjects((SaverType)t, scene, res);
		}
		...
		static savedGlobalId getSavedGlobalId(GameObject o)
		{
			...
			var namesPathBackup = o.name;
			var siblings = o.transform.GetSiblingIndex().ToString();
	...
	public class BookMarkCategory_Temp : IBgColor
	{
		...
		public Color? BgColor { get { return bgColor; } set { bgColor = value.Value; } }
	...
	[Serializable]
	public class IconExternalData : ICloneable
	{
		...
		public static Dictionary<string, Texture2D> icon_cacher = new Dictionary<string, Texture2D>();
		...
		{
			return (IconExternalData)MemberwiseClone();
		}
		...
		public Texture2D GetCheckedTexture()
		{
			...
			if (icon_cacher.ContainsKey(icon_guid)) return icon_cacher[icon_guid];
			if (icon_guid == null || icon_guid == "")
			{
				icon_cacher.Add(icon_guid, null);
			}
			else
			{
				bool _else = true;
				if (icon_guid.StartsWith("Library/"))
				{
					...
					if ((resource.StartsWith("sv_icon_dot") || resource.StartsWith("sv_label_")))
					{
						...
						icon_cacher.Add(icon_guid, t as Texture2D);
						_else = false;
					}
				}
				if (_else)
				{
					...
					icon_cacher.Add(icon_guid, t);
				}
			skip:;
			}
			return icon_cacher[icon_guid];
		}

		public void SetExternalCheckedTexture(Texture2D texture)
		{
			icon_cacher.Remove(icon_guid);
			if (!texture)
			{
				icon_path = "";
				icon_guid = "";
	...
	[Serializable]
	public class HighlighterExternalData
	{

		public int[] TempColorData = new int[0];
		...
		public void Load(string b64)
		{
			...
			{
				var split = arr.Split(' ');
				TempColorData = new int[split.Length];
				for (int i = 0; i < split.Length; i++) TempColorData[i] = int.Parse(split[i]);
			}
		}
	}
