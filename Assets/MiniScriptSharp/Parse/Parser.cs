	...
	public class Parser {
		...
		public void Parse(string sourceCode, bool replMode=false) {
			if (replMode) {
				...
				var isPartial = lastTok.Type switch {
					TokenType.EOL => false,
		...
		private void AllowLineBreak(Lexer tokens) {
			while (tokens.Peek().Type == TokenType.EOL && !tokens.AtEnd) tokens.Dequeue();
		}
		...
		private void ParseMultipleLines(Lexer tokens) {
			while (!tokens.AtEnd) {
				...
				if (tokens.Peek().Type == TokenType.Keyword && tokens.Peek().Text == END_FUNCTION) {
					tokens.Dequeue();
					if (outputStack.Count > 1) {
						outputStack.Pop();
						output = outputStack.Peek();
		...
		private void ParseStatement(Lexer tokens, bool allowExtra=false) {
			if (tokens.Peek().Type == TokenType.Keyword && tokens.Peek().Text != NOT
				...
				switch (keyword) {
					...
					case IF: {
							...
							if (tokens.Peek().Type != TokenType.EOL) {
								...
								if (tokens.Peek().Type == TokenType.Keyword && tokens.Peek().Text == ELSE) {
			...
			switch (peek.Type) {
				...
				default: {
					...
					while (true) {
						...
						if (tokens.Peek().Type == TokenType.Keyword && tokens.Peek().Text == ELSE) break;
						if (tokens.Peek().Type == TokenType.Comma) {
							tokens.Dequeue();
							AllowLineBreak(tokens);
							continue;
						}
						if (RequireEitherToken(tokens, TokenType.Comma, TokenType.EOL).Type == TokenType.EOL) break;
					}
		...
		private Value ParseFunction(Lexer tokens, bool asLval=false, bool statementStart=false) {
			...
			var tok = tokens.Peek();
			if (tok.Type != TokenType.Keyword || tok.Text != FUNCTION) return nextLevel(tokens, asLval, statementStart);
			tokens.Dequeue();
			...
			while (tokens.Peek().Type != TokenType.RParen) {
				...
				if (id.Type != TokenType.Identifier) throw new CompilerException(ErrorContext, tokens.LineNum,
		...
		private Value ParseIsA(Lexer tokens, bool asLval=false, bool statementStart=false) {
			...
			var val = nextLevel(tokens, asLval, statementStart);
			if (tokens.Peek().Type != TokenType.Keyword || tokens.Peek().Text != ISA) return val;
		...
		private Value ParseAddSub(Lexer tokens, bool asLval=false, bool statementStart=false) {
			...
			while (tok.Type == TokenType.OpPlus || 
				...
				output.Add(new Line(TAC.LTemp(tempNum), 
					tok.Type == TokenType.OpPlus ? Op.APlusB : Op.AMinusB,
					val, opB));
				val = TAC.RTemp(tempNum);
		...
		private Value ParseMultDiv(Lexer tokens, bool asLval=false, bool statementStart=false) {
			...
			var val = nextLevel(tokens, asLval, statementStart);
			var tok = tokens.Peek();
			while (tok.Type == TokenType.OpTimes || tok.Type == TokenType.OpDivide || tok.Type == TokenType.OpMod) {
				tokens.Dequeue();
		...
		private Value ParseUnaryMinus(Lexer tokens, bool asLval=false, bool statementStart=false) {
			...
			if (tokens.Peek().Type != TokenType.OpMinus) return nextLevel(tokens, asLval, statementStart);
			tokens.Dequeue();		// skip '-'
		...
		private Value ParseNew(Lexer tokens, bool asLval=false, bool statementStart=false) {
			...
			if (tokens.Peek().Type != TokenType.Keyword || tokens.Peek().Text != NEW) return nextLevel(tokens, asLval, statementStart);
		...
		private Value ParseAddressOf(Lexer tokens, bool asLval=false, bool statementStart=false) {
			...
			if (tokens.Peek().Type != TokenType.AddressOf) return nextLevel(tokens, asLval, statementStart);
			tokens.Dequeue();
			AllowLineBreak(tokens); // allow a line break after a unary operator
		...
		private Value ParseCallExpr(Lexer tokens, bool asLval=false, bool statementStart=false) {
			...
			while (true) {
				if (tokens.Peek().Type == TokenType.Dot) {
					...
					if (!asLval || (tokens.Peek().Type == TokenType.LParen && !tokens.Peek().AfterSpace)) {
		...
		private Value ParseMap(Lexer tokens, bool asLval=false, bool statementStart=false) {
			...
			if (tokens.Peek().Type != TokenType.LCurly) return nextLevel(tokens, asLval, statementStart);
			...
			if (tokens.Peek().Type == TokenType.RCurly) {
				...
				if (RequireEitherToken(tokens, TokenType.Comma, TokenType.RCurly).Type == TokenType.RCurly) break;
			}
			Value result = new ValTemp(output.nextTempNum++);
			output.Add(new Line(result, Op.CopyA, map));
			return result;
		...
		private Value ParseList(Lexer tokens, bool asLval=false, bool statementStart=false) {
			...
			if (tokens.Peek().Type != TokenType.LSquare) return nextLevel(tokens, asLval, statementStart);
			tokens.Dequeue();
			...
			if (tokens.Peek().Type == TokenType.RSquare) {
				...
				if (RequireEitherToken(tokens, TokenType.Comma, TokenType.RSquare).Type == TokenType.RSquare) break;
			}
		...
		private Value ParseQuantity(Lexer tokens, bool asLval=false, bool statementStart=false) {
			...
			if (tokens.Peek().Type != TokenType.LParen) return nextLevel(tokens, asLval, statementStart);
		...
		private Value ParseCallArgs(Value funcRef, Lexer tokens) {
			...
			if (tokens.Peek().Type == TokenType.LParen) {
				...
				if (tokens.Peek().Type == TokenType.RParen) {
					...
					output.Add(new Line(null, Op.PushParam, arg));
					argCount++;
					if (RequireEitherToken(tokens, TokenType.Comma, TokenType.RParen).Type == TokenType.RParen) break;
				}
			}
			var result = new ValTemp(output.nextTempNum++);
			output.Add(new Line(result, Op.CallFunctionA, funcRef, TAC.Num(argCount)));
		...
		private Token RequireToken(Lexer tokens, TokenType type, string text=null) {
			...
			if (got.Type == type && (text == null || got.Text == text)) return got;
